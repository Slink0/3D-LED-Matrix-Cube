...
from config import USE_VIRTUAL_GYRO, USE_MPU_STREAM
...
def main():
    ...
    if RUN_FLUID:
        ...
        # ── Gravity source priority ──────────────────────────────────────────
        # 1. USE_VIRTUAL_GYRO  — read from visualizer mouse rotation (dev mode)
        # 2. USE_MPU_STREAM    — read angles streamed from Pi over UDP (dev mode)
        # 3. Physical MPU6050  — read directly from sensor (final deployment)
        # 4. Fixed downward    — fallback if nothing else is available
